<template>
    <div class="controlesNodo" :style="estiloPos" @click="clickFuera" :class="{ deshabilitado: eliminandose }">
        <div id="contenedorAnuncios">
            <div id="anuncioCreandoDependencia" v-show="nodoCreandoDependencia">
                <div class="anuncio anuncioSeleccion">
                    <img v-show="!creandoDependencia" src="@/assets/iconos/codeBranch.svg" alt="Dependencia" />
            ...
            <div class="anuncio" :class="{
                    anuncioSeleccion: idNodoTarget === elNodo?.id,
                    deshabilitado: !nodoTargetRelevante,
                }" id="anuncioNodoTarget" v-show="idNodoTarget && (!elNodo || elNodo.id == idNodoTarget)"
                @click="$emit('centerEnTarget')" @mouseenter="$emit('hoveringAnuncioTarget', true)"
                @mouseleave="$emit('hoveringAnuncioTarget', false)" @touchstart="startTouchAnuncioTarget"
                @touchmove="moveTouchAnuncioTarget" @touchend="endTouchAnuncioTarget">
        ...
        <div id="nombre" ref="nombre" @click="toggleDespliege">
            <img v-if="elNodo && elNodo.tipoNodo === 'concepto'" src="@/assets/iconos/atlas/lightbulbEmpty.svg"
                alt="Skill" />
            ...
            <transition-group name="pan" @after-leave="recalcularHeights" tag="div" id="transicionFilas">
                <div class="filaControles" :class="[direccionTransicion]" key="fila1" v-show="filaMostrada === 2">
                    ...
                    <div class="bloqueControl" style="">
                        <div class="botonTexto selector botonControl" :class="{
                                deshabilitado: nodoAprendido,
                                checked: nodoFresco,
                            }" @click="marcarEstudiado" @dblclick="marcarAprendido" style="
                align-self: normal;
              ...
              ">
                        ...
                        <div class="botonTexto botonControl" style="
                border-top-left-radius: 0px;
                border-bottom-left-radius: 0px;
                align-self: normal;
              " v-show="datoUsuarioEsteNodo &&
                  datoUsuarioEsteNodo.estudiado &&
                  datoUsuarioEsteNodo.diasRepaso &&
                  !nodoAprendido
                  " @click.stop="mostrando = mostrando === 'tiempoRepaso' ? '' : 'tiempoRepaso'
        ">
                            <pie-progreso :mostrarNumero="false" :color-progreso="'#3f7d20'" :color-fondo="'transparent'"
                        ...
                        <div class="botonTexto selector botonControl" id="botonMarcarAprendido"
                            :class="{ checked: nodoAprendido }" style="width: 100%" @click="marcarAprendido">
                ...
                <div class="filaControles" :class="[direccionTransicion]" key="fila3" v-show="filaMostrada === 3">
        ...
        ">
                        ...
                        <div class="botonTexto" :class="{ deshabilitado: nodoCreandoDependencia }"
                            id="botonCrearDependenciaNodo" v-if="usuarioSuperadministrador || usuarioExpertoNodo"
                    ...
                    <div class="bloqueControl" v-if="usuarioSuperadministrador">
                        ...
                        <div class="boton botonControl" v-show="!eliminandose" @click="eliminarNodo">
                            <img src="@/assets/iconos/trash.svg" alt="Eliminar" />
                        </div>
                    </div>

                    <div class="contenidoMostrado" id="zonaGestionDependencias" v-show="mostrando === 'dependenciasNodo'">
                        <div id="listaDependencias" v-if="elNodo && elNodo.vinculos">
                            <div class="dependencia" v-for="vinculo of dependenciasNodo" :key="vinculo.id">
                                <nodo-conocimiento-vista-lista :idNodo="vinculo.idRef" :yo="yo" />
                                <div v-if="usuarioExperto || usuarioSuperadministrador"
                                    ...
                                    <img v-show="idVinculoEliminando != vinculo.id" src="@/assets/iconos/equis.svg"
                                        alt="Eliminar" />
                                </div>
                            </div>
                            <div class="anuncioZonaVacia" v-show="dependenciasNodo.length === 0">
                ...
                <div class="filaControles" :class="[direccionTransicion]" key="fila1" v-show="filaMostrada === 1">
                    <div id="zonaDescripcionNodo">
                        <div id="descripcionNodo">
                            {{ elNodo.descripcion || "" }}
                            <div class="anuncioZonaVacia" v-if="!elNodo?.descripcion?.length > 0">
                                Aún no hay descripción
                            </div>
                        </div>
                    </div>
                    <div class="bloqueControl">
                        <router-link v-if="elNodo?.id" :to="{
                    ...
                    <div class="bloqueControl" id="bloqueControlTarget">
                        <div class="botonTexto selector" :class="{ activo: elNodo.id === idNodoTarget }"
                            @click="setNodoTarget(elNodo.id)">
                            <img src="@/assets/iconos/crosshairsSolid.svg" alt="Mira" />
                            En la mira
                        </div>
                        <div class="botonTexto" v-show="elNodo.id === idNodoTarget" style="align-self: stretch"
            ...
            <div id="zonaSelectorFilas">
                <div class="selectorFila boton selector" :class="{ activo: filaMostrada === index }"
                    v-for="index of cantidadFilas" :key="'selectorFila' + index" @click.stop="setFilaMostrada(index)"></div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        desplegar() {
            console.log("desplegandoControles");
            if (!this.elNodo) {
                return;
            }
            if (this.desplegado) {
        ...
        recalcularHeights() {
            this.$nextTick(() => {
                ...
                this.heightAll = this.$el.clientHeight;
            });
        },
