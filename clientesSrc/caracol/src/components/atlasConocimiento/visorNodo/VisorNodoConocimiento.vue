    ...
    <div class="visorNodoConocimiento">
        ...
        <div id="zonaTitulo" v-show="!$apollo.queries.esteNodo.loading">
            ...
            <div id="zonaNombre">
                ...
                <input @keypress.enter.prevent="guardarNuevoNombre" ref="inputNuevoNombre" style="width: 250px"
                    @blur="guardarNuevoNombre" v-show="editandoNombre" type="text" class="inputNombreCosa"
                    id="inputNuevoNombre" />
            </div>
        </div>

        <div id="zonaSelectoresContenido" v-show="!$apollo.queries.esteNodo.loading">
            <div class="boton selectorContenido selector" :class="{ activo: mostrandoContenido === 'estudiar' }"
            ...
            <div class="boton selectorContenido selector" :class="{ activo: mostrandoContenido === 'expertos' }"
            ...
            <div class="boton selectorContenido selector deshabilitado" :class="{ activo: mostrandoContenido === 'foros' }"
                :title="mostrandoContenido != 'foros' ? 'Mostrar foros' : ''" @click="mostrandoContenido = 'foros'">
        ...
        <div id="zonaContenidos" v-show="!$apollo.queries.esteNodo.loading">
            <div id="zonaSecciones" v-show="mostrandoContenido === 'estudiar'">
                <div id="zonaTituloSeccion" v-show="!mostrandoMenuSecciones">
                    <div id="nombreSeccionSeleccionada" @click="mostrandoMenuSecciones = true">
                        <img class="iconoMenuSecciones" src="@/assets/iconos/stream.svg" alt="Menu"
                ...
                <div id="contenedorSelectoresSeccion" v-show="mostrandoMenuSecciones"
                    @click="mostrandoMenuSecciones = false">
                    <div class="selectorSeccion" @click="idSeccionVisible = null">
                        <div class="nombreSeccionSelector">
                            <div class="bolita" v-if="!idSeccionVisible"></div>
                            Descripcion
                        </div>
                    </div>
                    <div class="selectorSeccion" @click="idSeccionVisible = seccion.id"
                        :class="{ activa: idSeccionVisible === seccion.id }" v-for="(seccion, indexS) of esteNodo.secciones"
                        :key="seccion.id">
                        <div class="nombreSeccionSelector">
                            <div class="bolita" v-if="idSeccionVisible === seccion.id"></div>
                            {{ seccion.nombre }}
                        </div>
                        <div class="contenedorBotonesSelectorSeccion" v-if="usuarioExperto || usuarioSuperadministrador"
                            v-show="mostrandoBotonesSeccion === seccion.id" @click.stop="">
                            <div class="boton" @click="cambiarIndexSeccion(seccion.id, -1)"
                            ...
                            <div class="boton" title="Eliminar" v-show="usuarioExperto || usuarioSuperadministrador"
                                @click.stop="eliminarSeccion(seccion.id)">
                                <img src="@/assets/iconos/trash.svg" alt="Eliminar" />
                            </div>
                        </div>
                        <div class="boton botonMostrarBotonesSeccion" v-if="usuarioExperto || usuarioSuperadministrador"
                            @click.stop="mostrandoBotonesSeccion =
                                mostrandoBotonesSeccion === seccion.id ? null : seccion.id
                                ">
                            <img src="@/assets/iconos/ellipsisVertical.svg" alt="Opciones" />
                        </div>
                    </div>
                    <div class="selectorSeccion botonTexto" @click.stop="crearNuevaSeccion" v-if="usuarioExperto">
                        <loading v-show="creandoNuevaSeccion" />
                        <img src="@/assets/iconos/plusCircle.svg" alt="Plus" v-show="!creandoNuevaSeccion" />
                        <span>Crear sección</span>
                    </div>
                </div>
                <div id="zonaDescripcion" v-show="!seccionSeleccionada && !mostrandoMenuSecciones">
                    <div id="descripcion" class="contenidoTexto" ref="descripcion" v-show="!editandoDescripcion"
                        @click="toggleEditandoDescripcion">
                        {{ esteNodo.descripcion }}
                    </div>

                    <textarea id="inputNuevoDescripcion" ref="inputNuevoDescripcion" class="inputTextoNodo"
                        v-model="nuevoDescripcion" v-show="editandoDescripcion" />
                    <div class="contenedorBotonesCampo" v-show="editandoDescripcion">
                        <img src="@/assets/iconos/save.svg" class="botonGuardarCampo" alt="Guardar"
                            title="Guardar descripción" id="botonGuardarDescripcion" @click="guardarNuevoDescripcion" />
                        <img src="@/assets/iconos/equis.svg" class="botonGuardarCampo" alt="Cancelar"
                            title="Cancelar edición" id="botonCancelarEdicionDescripcion"
                            @click="editandoDescripcion = false" />
                    </div>
                </div>
                <seccion-nodo-conocimiento :key="seccionSeleccionada.id"
                    v-show="seccionSeleccionada && !mostrandoMenuSecciones" v-if="esteNodo.id && seccionSeleccionada"
                    :idNodo="esteNodo.id" :estaSeccion="seccionSeleccionada" :usuarioExperto="usuarioExperto"
        ...
        " />
            ...
            <div id="zonaExpertos" v-show="mostrandoContenido === 'expertos'">
                <div class="contenedorControles">
                    <div class="boton" v-show="!enviandoQueryExpertos &&
                        !idExpertoSeleccionado &&
                        !usuarioExperto
                        " :title="esteNodo.expertos.length < 1
            ? 'Asumir'
        ...
        " @click="intentarEntrarExpertos(usuario.id)">
                    ...
                    <div class="boton" v-show="!enviandoQueryExpertos &&
                        ...
                        " title="Aceptar como experto" @click="intentarEntrarExpertos(idExpertoSeleccionado)">
                    ...
                    <div class="boton" v-show="!enviandoQueryExpertos &&
                        ...
                        (usuarioExperto ||
                            esteNodo.posiblesExpertos.includes(idExpertoSeleccionado))
                        " title="abandonar" @click="abandonarListaExpertos(usuario.id)">
                ...
                <div id="listaExpertos" class="listaPersonas" @click.self="idExpertoSeleccionado = null">
                    <icono-persona-autonomo texto="" v-for="idExperto of esteNodo.expertos.concat(
                            esteNodo.posiblesExpertos
                        )" :seleccionado="idExpertoSeleccionado === idExperto" :key="idExperto" factorEscala="0.8"
                        :style="{
                                opacity: esteNodo.posiblesExpertos.includes(idExperto) ? 0.5 : 1,
                            }" :idPersona="idExperto" @click.native="idExpertoSeleccionado = idExperto" />
                </div>
            </div>

            <div id="zonaForos" ref="zonaForos" v-show="mostrandoContenido == 'foros'">
